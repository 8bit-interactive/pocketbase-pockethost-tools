...
```bash
python3 skills/pocketbase-pockethost/scripts/download_pocketbase.py
```
...
```bash
python3 skills/pocketbase-pockethost/scripts/download_pocketbase.py --version 0.37.1
```

To choose a different output directory:

```bash
python3 skills/pocketbase-pockethost/scripts/download_pocketbase.py --output-dir ./.tools/pocketbase
```
...
```bash
PB_BIN="$(python3 skills/pocketbase-pockethost/scripts/download_pocketbase.py --version 0.37.1 | tail -n 1)"
mkdir -p .tmp/pb-test
...
- Keep the binary download step and the hosted deployment step separate. Use `$pocketbase-pockethost` for the GitHub Actions and SFTP side.
